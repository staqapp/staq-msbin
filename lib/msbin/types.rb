...
def from_unicode(str)
  string.encode("UTF-8","UTF-16LE")#:invalid => :replace, :undef => :replace, :replace => "?")
  # to,from,str
...
module MSBIN
	...
	class PrefixElement < Element
		...
		def initialize(handle, record_type)
			$indent += 1
			#@name = "#{record_type-self.class.record_type.first+?a.ord}:" + read_string(handle)
